...
@section('title')
<h1> <i class='fa fa-bookmark' ></i> {{ trans('ecsa::titles.organization.index') }}</h1>
@endsection
...
<div class="col-lg-12" >
    <div class="row upper-menu " >
        <div style="float: right" >
            ...
            <a class="btn btn-info btn-new" href="{{ URL::route(\Config::get('ecsa::prefix').'.organization.create') }}">
                <i class='fa fa-plus' ></i> {{ trans('ecsa::btn.newm') }}</a>
        </div>
    </div>
        ...
        <table id="table" class="table table-striped table-hover table-bordered table-condensed" >
            ...
            <tbody>
                ...
                <tr {{ $color }} >
                    ...
                    <td class="visible-lg visible-md visible-sm visible-xs" >{{ Str::limit($orga->dir,40) }}</td>
                    <td class="visible-lg" >{{ $orga->municipio }}</td>
                    <td class="visible-md" >{{ $orga->estado }}</td>
                    <td><a href='{{ URL::route(\Config::get('ecsa::prefix').'.organization.index') }}/{{ \Crypt::encrypt($orga->id) }}' class="btn btn-primary" ><i class="fa fa-eye" ></i> Ver </a></td>
                </tr>
                @endforeach
            </tbody>
        </table>
...
<script src='{{ asset('packages/ecco/ecsa/js/organization.js') }}' ></script>
@endsection